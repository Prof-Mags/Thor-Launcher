...
internal fun ProfilesPage(
    ...
    registry.profiles.forEach { profile ->
        ...
        /*
         * Editing rows only for the profile in use.
         *
         * Renaming or re-picturing somebody else's profile from inside your own
         * is a strange power to have, and repeating four rows per profile would
         * make a page navigated one row at a time very long indeed.
         */
        if (isActive) {
            ...
            TextFieldRow(
                title = "Name",
                subtitle = "Shown on the top screen and in this list",
                value = profile.name,
...
/**
 * Focusable rows on the profiles page.
 *
 ...
 */
internal fun profilesRowCount(registry: ProfileRegistry, activeHasAvatar: Boolean): Int {
    val activeId = registry.active?.id
    val perProfile = registry.profiles.fold(0) { total, profile ->
        total + when {
            // Switch, picture, name, colour — plus remove, when there is one.
            profile.id == activeId -> if (activeHasAvatar) 5 else 4
            // Switch, then delete.
            else -> 2
        }
    }
